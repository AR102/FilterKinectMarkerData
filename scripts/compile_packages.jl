...
dir = splitdir(pwd())[2]
# if in FilterKinect.jl/scripts (because called by another script etc.), go back to main dir
if dir == "scripts"
    cd("..")
end
...
# joinpath necessary because include uses location of executed file as base for relative
# paths, not the current working directory, see
...
PackageCompiler.create_sysimage(
    used_packages; sysimage_path="JuliaSysimage.$(Libdl.dlext)", project="."
)